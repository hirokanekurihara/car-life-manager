...
function App() {
  ...
  const [vehicleViewMode, setVehicleViewMode] = useState('admin');
  ...
  const [vehicleEditForm, setVehicleEditForm] = useState({
    ...
    taxDue: '', inspectionDue: '', insuranceExpiry: '',
    insuranceType: '', insuranceCompany: '', insurancePerson: '', insurancePhone: '', insurancePolicyNo: '',
    taxPaid: false
  });
  ...
  async function addVehicle(e) {
    ...
    await db.vehicles.add({
      ...
      insuranceCompany: fd.get('insuranceCompany') || '',
      insurancePerson: fd.get('insurancePerson') || '',
      insurancePhone: fd.get('insurancePhone') || '',
      insurancePolicyNo: fd.get('insurancePolicyNo') || '',
      photoBlob,
      updatedAt: Date.now()
    });
  ...
  function goToVehicleDetail(vehicle) {
    ...
    setVehicleViewMode('detail');
    setTab('vehicles');
    setTimeout(() => {
      const el = document.getElementById('vehicle-detail-panel');
  ...
  function openTab(nextTab) {
    setTab(nextTab);
    if (nextTab === 'vehicles') setVehicleViewMode('admin');
  }
  ...
  function editVehicleBasic(v) {
    ...
    setVehicleEditForm({
      ...
      insurancePerson: v.insurancePerson || '',
      insurancePhone: v.insurancePhone || '',
  ...
  function renderVehicles() {
    ...
    return (
      <section className="space-y-4">
        <article id="vehicle-detail-panel" className="card p-4 space-y-3">
          <h2 className="text-lg font-extrabold">車両詳細</h2>
          {selectedVehicle ? (
            <>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">車種:</span> {selectedVehicle.name || '-'}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">ナンバー:</span> {selectedVehicle.plate || '-'}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">年式:</span> {selectedVehicle.year || '-'}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">走行距離:</span> {Number(selectedVehicle.currentMileage || 0).toLocaleString()} km</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">任意保険満了:</span> {formatDate(selectedVehicle.insuranceExpiry)}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">保険種別:</span> {selectedVehicle.insuranceType || '-'}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">保険会社:</span> {selectedVehicle.insuranceCompany || '-'}</div>
                <div className="rounded border bg-gray-50 px-3 py-2"><span className="text-gray-500">保険電話:</span> {selectedVehicle.insurancePhone ? <a className="text-blue-600" href={`tel:${selectedVehicle.insurancePhone}`}>{selectedVehicle.insurancePhone}</a> : '-'}</div>
                <div className="rounded border bg-indigo-50 px-3 py-2"><span className="text-gray-500">NOJチケット残数:</span> {Number(selectedVehicle.nojTicketRemaining || 0)} 枚</div>
                <div className="rounded border bg-gray-50 px-3 py-2 col-span-2"><span className="text-gray-500">証券番号:</span> {selectedVehicle.insurancePolicyNo || '-'}</div>
              </div>
            ...
            <p className="text-sm text-gray-500">ヘッダーまたはホーム画面で対象車両を選択すると、詳細と履歴が表示されます。</p>
          )}
        </article>
          ...
          {showVehicleRegisterForm && (
            <form id="vehicle-register-form" className="space-y-3" onSubmit={addVehicle}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <Field label="車種名">
                  <input className="input" name="name" placeholder="例: プリウス" required />
                </Field>
                <Field label="ナンバー">
                  <input className="input" name="plate" placeholder="例: 品川300 あ 12-34" />
                </Field>
                <Field label="年式">
                  <input className="input" name="year" type="number" min="1900" max="2100" placeholder="例: 2022" />
                </Field>
                <Field label="現在走行距離(km)">
                  <input className="input" name="currentMileage" type="number" min="0" placeholder="例: 24500" />
                </Field>
                <Field label="排気量(cc)">
                  <input className="input" name="displacementCc" type="number" min="0" placeholder="例: 1800" />
                </Field>
                <Field label="現在装着タイヤ">
                ...
                <Field label="車検満了日">
                  <input className="input" name="inspectionDue" type="date" />
                </Field>
                <Field label="任意保険満了日">
                  <input className="input" name="insuranceExpiry" type="date" />
                </Field>
                <Field label="自動車税納付期限">
                  <input className="input" name="taxDue" type="date" />
                </Field>
                <Field label="保険種別（任意）">
                  <input className="input" name="insuranceType" placeholder="例: 任意" />
                </Field>
                <Field label="保険会社（任意）">
                  <input className="input" name="insuranceCompany" placeholder="例: ○○損保" />
                </Field>
                <Field label="保険電話（任意）">
                  <input className="input" name="insurancePhone" placeholder="例: 03-xxxx-xxxx" />
                </Field>
                <Field label="証券番号（任意）">
                  <input className="input" name="insurancePolicyNo" placeholder="例: ABCD-1234" />
                </Field>
                <Field label="車両写真">
      ...
      <section className="space-y-4">
        ...
        <article className="card p-4 space-y-2">
          ...
          <div className="space-y-1 text-sm">
            {insuranceContacts.map((item) => (
              <div key={item.id} className="flex items-center justify-between gap-2 rounded border bg-gray-50 px-3 py-2">
                <p>
        ...
        <article className="card p-4 space-y-3 text-sm">
          ...
          {insurances.map((item) => {
            ...
            return (
              <div key={item.id} className="rounded-lg border bg-gray-50 p-3 space-y-1">
                ...
                <div className="pt-1">
                  <button className="px-3 py-1 rounded bg-red-100 text-red-700" onClick={() => deleteInsurancePolicy(item.id)}>削除</button>
                </div>
              </div>
            );
          })}
    ...
    <div className="app-shell max-w-7xl mx-auto px-3 md:px-6 py-3 md:py-6">
      ...
      <nav className="bottom-nav fixed bottom-0 left-0 right-0 bg-white/95 border-t z-20 md:hidden">
        <ul className="grid" style={{ gridTemplateColumns: `repeat(${APP_TABS.length}, minmax(0, 1fr))` }}>
          {APP_TABS.map((t) => (
            <li key={t.key}>
              <button className={`bottom-nav-btn w-full py-2 text-xs ${tab === t.key ? 'text-indigo-600 font-bold' : 'text-gray-500'}`} onClick={() => setTab(t.key)}>
