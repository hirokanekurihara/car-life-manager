...
const CLOUD_SYNC_TABLE = 'appSyncSnapshots';
const CLOUD_SYNC_ID = 'default';
const CLOUD_SYNC_DEVICE_KEY = 'cloudSyncDeviceName';
const CLOUD_SYNC_LAST_AT_KEY = 'cloudSyncLastAt';
const APP_BUILD_VERSION = '20260508-5';
...
function App() {
  ...
  const [syncDeviceName, setSyncDeviceName] = useState(() => localStorage.getItem(CLOUD_SYNC_DEVICE_KEY) || '');
  const [syncBusy, setSyncBusy] = useState(false);
  const [syncStatus, setSyncStatus] = useState('');
  const [lastCloudSyncAt, setLastCloudSyncAt] = useState(() => localStorage.getItem(CLOUD_SYNC_LAST_AT_KEY) || '');
  ...
  async function restoreFromBackupPayload(data, askConfirm = true) {
    if (!data) return;
    if (askConfirm && !confirm('既存データをすべて置き換えて復元します。実行しますか？')) return;
    ...
    await db.transaction('rw', db.vehicles, db.maintenances, db.fuelLogs, db.tireLogs, db.memos, db.categories, db.contacts, db.shops, db.insuranceContacts, db.insurances, async () => {
      ...
      if (vehiclesRestored.length) await db.vehicles.bulkAdd(vehiclesRestored);
      if (maintenanceRestored.length) await db.maintenances.bulkAdd(maintenanceRestored);
      if ((data.fuelLogs || []).length) await db.fuelLogs.bulkAdd(data.fuelLogs || []);
      if ((data.tireLogs || []).length) await db.tireLogs.bulkAdd(data.tireLogs || []);
      if (memosRestored.length) await db.memos.bulkAdd(memosRestored);
      ...
      if ((data.insuranceContacts || []).length) await db.insuranceContacts.bulkAdd(data.insuranceContacts || []);
      if (insurancesRestored.length) await db.insurances.bulkAdd(insurancesRestored);
    });
  ...
  async function restoreAll(file) {
    ...
    if (!backupFile) {
      alert('backup.json が見つかりません。正しいバックアップZIPを選択してください。');
      return;
    }
    const text = await backupFile.async('string');
    const data = JSON.parse(text);
    await restoreFromBackupPayload(data, true);
    alert('復元が完了しました。');
  }

  async function fetchCloudSyncRecord() {
    const res = await fetch(`tables/${CLOUD_SYNC_TABLE}/${encodeURIComponent(CLOUD_SYNC_ID)}`);
    if (res.status === 404) return null;
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(`クラウド同期レコード取得失敗: ${res.status} ${text}`);
    }
    return res.json();
  }
  ...
  async function uploadCloudSync() {
    ...
    try {
      ...
      const body = {
        ...
        device: syncDeviceName || '未設定端末',
        updatedAt: now,
        version: APP_BUILD_VERSION
      };
      const exists = await fetchCloudSyncRecord();
      const url = exists ? `tables/${CLOUD_SYNC_TABLE}/${encodeURIComponent(CLOUD_SYNC_ID)}` : `tables/${CLOUD_SYNC_TABLE}`;
      ...
      if (!saveRes.ok) {
        ...
        throw new Error(`クラウド保存失敗: ${saveRes.status} ${text}`);
      }

      const stamp = String(now);
      localStorage.setItem(CLOUD_SYNC_LAST_AT_KEY, stamp);
      setLastCloudSyncAt(stamp);
      setSyncStatus(`アップロード完了: ${new Date(now).toLocaleString('ja-JP')}`);
    } catch (error) {
      console.error(error);
      setSyncStatus(`アップロード失敗: ${error.message || '不明なエラー'}`);
      alert('クラウドへのアップロードに失敗しました。ネットワーク接続と同期設定を確認してください。');
    } finally {
      setSyncBusy(false);
    }
  }
  ...
  async function downloadCloudSync() {
    ...
    try {
      const record = await fetchCloudSyncRecord();
      if (!record || !record.payload) {
        alert('クラウドに同期データがありません。先にPC側で「クラウドへ保存」を実行してください。');
        setSyncStatus('クラウドデータなし');
        return;
      }

      if (!confirm('クラウドの保存データで現在の端末データを置き換えます。実行しますか？')) {
        setSyncStatus('ダウンロードをキャンセルしました。');
        return;
      }

      const data = JSON.parse(record.payload);
      await restoreFromBackupPayload(data, false);

      const stamp = String(Date.now());
      localStorage.setItem(CLOUD_SYNC_LAST_AT_KEY, stamp);
      setLastCloudSyncAt(stamp);
      setSyncStatus(`ダウンロード完了: ${new Date().toLocaleString('ja-JP')}（保存元: ${record.device || '不明'}）`);
      alert('クラウド同期の反映が完了しました。');
    } catch (error) {
      console.error(error);
      setSyncStatus(`ダウンロード失敗: ${error.message || '不明なエラー'}`);
      alert('クラウドからの取得に失敗しました。ネットワーク接続を確認して再試行してください。');
    } finally {
      setSyncBusy(false);
    }
  }
    ...
    return (
      <section className="space-y-4">
        ...
        <article className="card p-4 space-y-3">
          ...
          <button type="button" className="w-full py-2 rounded-xl bg-green-600 text-white font-bold" onClick={backupAll}>完全バックアップ（ZIP）</button>
          <button type="button" className="w-full py-2 rounded-xl bg-emerald-600 text-white font-bold" onClick={exportCsvZip}>CSVエクスポート（ZIP）</button>
          ...
          <div className="rounded-lg border bg-gray-50 p-3 space-y-2">
            <h3 className="font-semibold text-sm">PC / iPhone クラウド同期（手動）</h3>
            <p className="text-xs text-gray-600">同じURLでも端末ごとにデータは別保存です。下のボタンでクラウド同期できます。</p>
            <Field label="端末名（任意）">
              <input
                className="input"
                ...
                onChange={(e) => {
                  ...
                  setSyncDeviceName(next);
                  localStorage.setItem(CLOUD_SYNC_DEVICE_KEY, next);
                }}
                placeholder="例: PC-自宅 / iPhone-山田"
              />
            </Field>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <button type="button" className="py-2 rounded-xl bg-indigo-600 text-white font-bold disabled:bg-gray-300" onClick={uploadCloudSync} disabled={syncBusy}>
                {syncBusy ? '処理中...' : 'この端末のデータをクラウドへ保存'}
              </button>
              <button type="button" className="py-2 rounded-xl bg-blue-600 text-white font-bold disabled:bg-gray-300" onClick={downloadCloudSync} disabled={syncBusy}>
                {syncBusy ? '処理中...' : 'クラウドのデータをこの端末へ反映'}
              </button>
            </div>
            {lastCloudSyncAt && <p className="text-xs text-gray-500">最終同期: {new Date(Number(lastCloudSyncAt)).toLocaleString('ja-JP')}</p>}
            {syncStatus && <p className="text-xs text-indigo-700">{syncStatus}</p>}
          </div>
        </article>
      </section>
    );
